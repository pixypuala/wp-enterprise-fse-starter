...
- Capability checks at every privileged action; nonces verify intent but never replace authorization.
- Escape at output, sanitize and validate at boundaries, use prepared SQL, and prohibit direct file access where relevant.
- No arbitrary file upload, remote URL fetch, unserialize, dynamic include, eval, or user-controlled callback execution.
- REST responses follow least disclosure; unpublished content and private metadata require explicit permission.
- Dependency and build provenance are documented; release artifacts are reproducible and exclude development secrets.
